...
for to in range (1, len (x) - 99, 100):
    # cut-off frequency oscillating with 0.91 Hz between 100 and 1900Hz (LFO, low frequency oscillation)
    fc = (1000 + math.sin (to * 0.71 * 2 * math.pi / fs) * 900) / fs
    fd = 0.2 * fc  #the bandwidth is set at a fifth of the cut-off frecency

    # filter parameter depending on cut-off frequency
    c = -math.cos(2*math.pi *fc)
    d = (math.tan (math.pi * fd) - 1) / (math.tan (math.pi * fd) + 1)

    m = 0.2
    q = 0.9

    '''
    m in this case is a mixing parameter that its value ranges
    from 0 to 1. Such that, when m is 0, the output signal y[t] is 
    the same as the input signal x[t]. And when m is 1, the output signal.

    q is the feedback gain. When q is 0, it means there is no feeback gain. 
    However, when it is 1, it simply means the feeback is exactly the previous
    output. 
    '''
    ...
    for ti in range (0, 100):
      ...
      x[t] = np.clip(float(x[t]) + (q * feedback), -32768, 32767).astype(np.int16)

      a0 = -1 * d * x[t] + c *(1-d)*x[t - 1] + x[t-2] - c * (1-d)* a1  + d *a2 #output of second-order all-pass filter

      y[t] = ((1-m) * x[t]) + (m * a0) # phaser from first band-reject filter via second-order allpass filter
      #we cascade four  together, with the first band-reject filter is used as input for the second band-reject filter.
      y[t] = ((1-m)* y[t] + (m *a0))  # phase from band-reject filter via second-order allpass filter
      y[t] = ((1-m)* y[t] + (m *a0)) # phase from band-reject filter via second-order allpass filter
      y[t] = ((1-m)* y[t] + (m *a0))  # phase from band-reject filter via second-order allpass filter
...
wavfile.write("output_extended_phaser.wav", fs, y)
