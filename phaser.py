...
for to in range (1, len (x) - 99, 100):
    ...
    the same as the input signal x[t]. And when m is 1, the out signal 
    '''
    ...
    for ti in range (0, 100):
      ...
      y[t] = ((1-m) * x[t]) + (m * a0)  # phaser from band-reject filter via second-order allpass filter
...
wavfile.write("output_phaser.wav", fs, y)
